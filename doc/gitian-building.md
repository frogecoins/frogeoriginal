...
*Setup instructions for a Gitian build of Froge Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Froge
...
- [Building Froge Core](#building-froge-core)
...
Clone the git repositories for froge and Gitian.
...
in the froge repository under 'Fetch and create inputs' to install sources which require
...
To build Froge Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the froge repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/froge/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/froge-project/froge
...
For example:
```bash
URL=https://github.com/thrasher-/froge.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit froge=${COMMIT} --url froge=${URL} ../froge/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit froge=${COMMIT} --url froge=${URL} ../froge/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit froge=${COMMIT} --url froge=${URL} ../froge/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the froge git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../froge/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/froge
SIGPATH=/some/root/path/froge-detached-sigs

./bin/gbuild --url froge=${BTCPATH},signature=${SIGPATH} ../froge/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/froge-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/froge-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/froge-osx-build.assert
```
...
[froge-project/gitian.sigs.ltc](https://github.com/froge-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
